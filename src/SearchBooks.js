      ...
      <div className="search-books">
        <div className="search-books-bar">
          ...
          <div className="search-books-input-wrapper">
            <input type="text" placeholder="Search By Book Author Or Title" onChange={(event) => this.updateQuery(event.target.value)} />
          </div>
        </div>
        <div className="search-books-results">
          <ol className="books-grid">
            {bookCollection.map((book) => (
              <Book key={book.id} book={book} shelfSwitch={this.props.shelfSwitch}/>
            ))}
          </ol>
        </div>
      </div>
